...
import {
  PublicKey, LAMPORTS_PER_SOL, SystemProgram, Keypair,
...
export default function CreateBoarding({ onBack, onCreate }: CreateBoardingProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const supply = parseInt(tokenSupply || '1000000000');
      const supplyRaw = BigInt(supply) * BigInt(10 ** TOKEN_DECIMALS);

      // ── TX1: Create mint + ATA + mint tokens ──
      setTxStep('Creating token mint...');
      ...
      const tx1 = new Transaction().add(
        ...
        // Initialize mint (6 decimals, creator = authority)
        createInitializeMint2Instruction(
          mint, TOKEN_DECIMALS, publicKey, null, TOKEN_PROGRAM_ID
        ),
        // Create ATA for creator
        createAssociatedTokenAccountInstruction(
          publicKey, creatorAta, publicKey, mint,
          TOKEN_PROGRAM_ID, ASSOCIATED_TOKEN_PROGRAM_ID
        ),
        // Mint full supply to creator
        createMintToInstruction(
          mint, creatorAta, publicKey, supplyRaw, [], TOKEN_PROGRAM_ID
        ),
      );
      ...
      // ── TX2: Create boarding pool ──
      setTxStep('Creating boarding pool...');

      const tickerUpper = tokenSymbol.toUpperCase().padEnd(10, '\0').slice(0, 10);

      const [poolPda] = PublicKey.findProgramAddressSync(
        [Buffer.from('boarding_pool'), mint.toBuffer(), publicKey.toBuffer()],
        programId
      );
      ...
      const [tickerClaim] = PublicKey.findProgramAddressSync(
        [Buffer.from('ticker'), Buffer.from(tokenSymbol.toUpperCase())],
        programId
      );

      const tx2 = await program.methods
        .createPool(
          new BN(hardCap * LAMPORTS_PER_SOL),
          new BN(perWalletCap * LAMPORTS_PER_SOL),
          new BN(MODE_DURATIONS[mode]),
          new BN(supplyRaw.toString()),
          access === 'crew' ? { crew: {} } : { public: {} },
      ...
      setTxStatus(`Pool created! Ticker: $${tokenSymbol.toUpperCase()}`);
      setTxStep('');
  ...
  return (
    <div className="fade-up">
      <button onClick={onBack} className="text-[13px] text-text-muted hover:text-primary transition-colors mb-6 cursor-pointer">
        &larr; BACK
      </button>
      ...
      <div className="flex mb-6 relative">
        ...
        {['Token', 'Config', 'Launch'].map((label, i) => (
          <div key={i} className="flex-1 flex flex-col items-center z-[1]">
            ...
            <span className={`mt-2 text-[11px] tracking-[1px] ${step === i + 1 ? 'text-primary' : 'text-text-dim'}`}>
              {label}
            </span>
          </div>
        ))}
      ...
      <div className="rounded-xl p-6 bg-bg-glass border border-[rgba(136,192,255,0.08)]">
        ...
        {step === 1 && (
          <div className="fade-in">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-[11px] text-primary mb-1.5 tracking-[2px]">NAME *</label>
                <input type="text" value={tokenName} onChange={(e) => setTokenName(e.target.value)}
                  placeholder="e.g. MOONBASE" className={inputCls} />
              </div>
              <div>
                <label className="block text-[11px] text-primary mb-1.5 tracking-[2px]">TICKER *</label>
                ...
                <p className="text-[11px] text-text-dim mt-1">1-10 characters. Must be unique across all pools.</p>
              </div>
              <div className="col-span-2">
                <label className="block text-[11px] text-primary mb-1.5 tracking-[2px]">TOTAL SUPPLY *</label>
                <input type="number" value={tokenSupply} onChange={(e) => setTokenSupply(e.target.value)}
                  placeholder="1000000000" className={inputCls} />
                <p className="text-[11px] text-text-dim mt-1">60% presale &middot; 35% LP (burned) &middot; 5% dev bag</p>
              </div>
              <div>
                <label className="block text-[11px] text-primary mb-1.5 tracking-[2px]">IMAGE</label>
                <div className="h-[100px] bg-bg-input border-2 border-dashed border-[rgba(136,192,255,0.15)] rounded-lg flex flex-col items-center justify-center text-text-dim text-[12px] cursor-pointer hover:border-primary/30 transition-colors">
                  <span className="text-2xl mb-1 opacity-40">{'\uD83D\uDEA2'}</span>
                  Drop or click
                </div>
              </div>
              <div>
                <label className="block text-[11px] text-primary mb-1.5 tracking-[2px]">SOCIALS</label>
        ...
        {step === 2 && (
          <div className="fade-in">
            ...
            <div className="mb-5 p-4 bg-bg-input rounded-xl border border-[rgba(136,192,255,0.08)]">
              <div className="text-[11px] text-text-dim tracking-[2px] mb-3">V1 FIXED PARAMETERS</div>
              <div className="grid grid-cols-3 gap-4">
                <div>
                  <div className="text-[11px] text-text-dim tracking-[1px] mb-0.5">HARD CAP</div>
                  <div className="text-lg font-heading font-bold text-white tabular-nums">{hardCap} SOL</div>
                </div>
                <div>
                  <div className="text-[11px] text-text-dim tracking-[1px] mb-0.5">PER WALLET</div>
                  <div className="text-lg font-heading font-bold text-white tabular-nums">{perWalletCap} SOL</div>
                </div>
                <div>
                  <div className="text-[11px] text-text-dim tracking-[1px] mb-0.5">MIN WALLETS</div>
            ...
            {/* Mode */}
            <label className="block text-[11px] text-primary mb-2 tracking-[2px]">MODE</label>
            ...
            {/* Access */}
            <label className="block text-[11px] text-primary mb-2 tracking-[2px]">ACCESS</label>
            <div className="grid grid-cols-2 gap-3">
              ...
              >
                ...
                <div className="text-[12px] text-text-muted">Open to everyone</div>
              </div>
              <div
              ...
              >
                ...
                <div className="text-[12px] text-text-muted">Invite-only whitelist</div>
              </div>
            </div>

            {access === 'crew' && (
              <div className="mt-3 p-3.5 bg-bg-input rounded-xl border border-[#34d399]/15">
                <label className="block text-[11px] text-[#34d399] mb-1.5 tracking-[2px]">CREW LIST</label>
                <textarea
                  value={crewList}
                ...
                />
                <p className="text-[11px] text-text-dim mt-1">{crewCount} member{crewCount !== 1 ? 's' : ''}</p>
              </div>
            )}
          </div>
        )}
        ...
        {step === 3 && (
          <div className="fade-in">
            ...
            {txStatus && (
              <div className={`mb-4 p-3 rounded-lg text-[13px] font-mono ${
            ...
            {txStep && (
              <div className="mb-4 p-3 rounded-lg text-[13px] font-mono bg-primary/8 border border-primary/20 text-primary animate-pulse">
                {txStep}
              </div>
            )}
            ...
            <div className="grid grid-cols-[1.4fr_1fr] gap-4">
              ...
              <div className="p-4 bg-bg-input rounded-xl border border-[rgba(136,192,255,0.08)]">
                <div className="text-[11px] text-text-dim tracking-[2px] mb-3">MANIFEST</div>
                {[
                  ...
                  <div key={i} className="flex justify-between py-2 border-b border-[rgba(136,192,255,0.06)] last:border-0">
                    <span className="text-[12px] text-text-muted">{row.k}</span>
                    <span className="text-[12px] text-white font-semibold">{row.v}</span>
                  </div>
                ))}
              </div>
              ...
              <div>
                ...
                <div className="p-3.5 bg-burn/5 border border-burn/15 rounded-xl mb-3">
                  <div className="text-[11px] text-burn tracking-[2px] mb-2">CREATION FEE</div>
                  <div className="flex justify-between items-baseline">
                    <span className="text-[12px] text-text-muted">Non-refundable</span>
                    <span className="text-base font-heading font-bold text-burn">{CREATION_FEE_SOL} SOL</span>
                  </div>
                  <p className="text-[11px] text-text-dim mt-1.5">Paid upfront to open the pool. You get it back 4x if funded (2.5% creator fee on {hardCap} SOL = {hardCap * 0.025} SOL).</p>
                </div>
                ...
                <div className="glow p-4 bg-gradient-to-br from-[rgba(136,192,255,0.1)] to-[rgba(136,192,255,0.03)] border border-primary/15 rounded-xl mb-3">
                  <div className="text-[11px] text-primary tracking-[2px] mb-2">ON SUCCESS</div>
                  <div className="space-y-1.5">
                    <div className="text-[11px] text-text-dim tracking-[1px] mb-1">SOL SPLIT</div>
                    <div className="flex justify-between">
                      <span className="text-[12px] text-text-dim">92.5% &rarr; Raydium LP</span>
                      <span className="text-[12px] text-primary font-semibold">{(hardCap * 0.925).toFixed(1)} SOL</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-[12px] text-text-dim">5% &rarr; Platform</span>
                      <span className="text-[12px] text-white">{(hardCap * 0.05).toFixed(1)} SOL</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-[12px] text-text-dim">2.5% &rarr; You</span>
                      <span className="text-[12px] text-success font-semibold">{(hardCap * 0.025).toFixed(1)} SOL</span>
                    </div>
                    <div className="h-px bg-[rgba(136,192,255,0.08)] my-1" />
                    <div className="text-[11px] text-text-dim tracking-[1px] mb-1">TOKEN SPLIT</div>
                    <div className="flex justify-between">
                      <span className="text-[12px] text-text-dim">60% &rarr; Presale buyers</span>
                      <span className="text-[12px] text-white font-semibold">{((parseInt(tokenSupply || '0') * 0.6) / 1e6).toFixed(0)}M</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-[12px] text-text-dim">35% &rarr; LP (burned)</span>
                      <span className="text-[12px] text-primary font-semibold">{((parseInt(tokenSupply || '0') * 0.35) / 1e6).toFixed(0)}M</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-[12px] text-text-dim">5% &rarr; You (dev bag)</span>
                      <span className="text-[12px] text-success font-semibold">{((parseInt(tokenSupply || '0') * 0.05) / 1e6).toFixed(0)}M</span>
                    </div>
                  </div>
                </div>

                <div className="p-2.5 bg-bg-input border border-[rgba(136,192,255,0.08)] rounded-lg mb-3">
                  <div className="text-[12px] text-text-dim">On failure: depositors get 100% refund. Creation fee is not returned.</div>
                </div>
                ...
                >
                  {!connected ? 'CONNECT WALLET' : submitting ? txStep || 'LAUNCHING...' : 'LAUNCH BOARDING'}
                </button>
                <p className="text-[11px] text-text-dim text-center mt-1.5">Devnet &middot; 2 transactions (mint + pool)</p>
              </div>
            </div>
          </div>
        )}
        ...
        <div className="flex justify-between mt-6 pt-4 border-t border-[rgba(136,192,255,0.08)]">
          <button
            ...
            disabled={step === 1}
            className={`px-5 py-2.5 bg-transparent border border-border-primary rounded-lg text-[12px] ${
          ...
          {step < 3 && (
            <button
              ...
              className="px-5 py-2.5 bg-gradient-to-br from-primary to-primary-dark text-bg-base border-none rounded-lg text-[12px] font-semibold cursor-pointer"
            >
              CONTINUE &rarr;
            </button>
          )}
